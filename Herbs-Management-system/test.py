...
def customer(customerdestroy):
    def search_herbs(self):
        ...
        e5 = Entry(search_window) #for data input
        e5.place(x=200, y=80)

        l21 = Button(search_window, text="Search", command=lambda: [search_ex(e5.get())])
        l21.place(x=250, y=150)
    ...
    def search_ex(result):
        ...
        if mycursor.rowcount == 1:
            ...
            Herb_window1 = Tk()
            Herb_window1.title('Herb Information')
            ...
            message_box.showwarning('Error', 'OOPS! Invalid Input.. Please try again..')

    def show_herbs(self):
        Herb_window = Tk()
        self.destroy()

        Herb_window.title('Herbs Information')
        ...
        back_menu4 = Button(Herb_window, text='BACK', command=lambda: [customer(newWindow1)])
        back_menu4.grid(row=i+3, column=4)

    def order_lists1(self):
        insert_window = Tk()
        self.destroy()
        insert_window.geometry("600x500")
        insert_window.title('Order Menu')
    ...
    def herbs_insert():
        ...
        herbs = Button(insert_window, text="OK", command=lambda: fetch_herb_id(a.get(), b.get()))
    ...
    newWindow1 = Tk()
    newWindow1.title('MAIN MENU')
    customerdestroy.destroy()
    newWindow1.geometry("500x300")

    show_inventory = Button(newWindow1, text="Show All herbs", command=lambda: [show_herbs(newWindow1)])
    show_inventory.place(x=100, y=40)
    show_order_lists = Button(newWindow1, text='Order', command=lambda: [order_lists1(newWindow1)])
    show_order_lists.place(x=220, y=40)
    search_herbs1 = Button(newWindow1, text='Search', command=lambda: [search_herbs(newWindow1)])
    search_herbs1.place(x=300, y=40)

    search_herbs3 = Button(newWindow1, text='BACK', command=lambda: [login_user(newWindow1)])
    search_herbs3.place(x=220, y=100)
...
def login_user(self):
    ...
    if mycursor.rowcount == 1:
        def inventory(self2):
            ...
            btn = Button(Herb_window, text="Add herb", command=lambda: [add_product(Herb_window)])
            btn.grid(row=i + 2, column=1)

            btn = Button(Herb_window, text="Delete herb", command=lambda: [remove_product(Herb_window)])
            btn.grid(row=i + 2, column=2)

            btn = Button(Herb_window, text="Restock Herbs", command=lambda: [restock(Herb_window)])
        ...
        def add_product(self):
            ...
            insert_window.title('Add Herbs')
            insert_window.geometry("900x500")
        ...
        def remove_product(self):
            ...
            self.destroy()
            insert_window.title('Delete Herbs')
        ...
        def restock(self):
            ...
            self.destroy()
            another_window.title('Restock Herbs')
        ...
        def order_lists(self2):
            order_window = Tk()
            self2.destroy()
            order_window.title('Order Information')
            ...
            btn = Button(order_window, text="OK", command=lambda: show_order(order_entry.get()))
        ...
        def users(newWindowdestroy):
            user_window = Tk()
            newWindowdestroy.destroy()
            ...
            create_user = Button(user_window, text='ADD USER', command=lambda: [insert_user(user_window)])
            create_user.grid(row=1, column=6)

            del_user = Button(user_window, text='DELETE USER', command=lambda: [delete_user(user_window)])
            del_user.grid(row=2, column=6)

        def insert_user(self3):
            insert_window = Tk()
            self3.destroy()
            insert_window.geometry("500x300")
            insert_window.title('Admin Menu')
        ...
        def delete_user(self3):
            insert_window = Tk()
            self3.destroy()

            insert_window.title('Delete User')
            insert_window.geometry("500x300")
        ...
        show_inventory = Button(newWindow, text="Show herbs", command=lambda: [inventory(newWindow)])
        show_inventory.place(x=20, y=20)

        show_order_lists = Button(newWindow, text='Show order lists', command=lambda: [order_lists(newWindow)])
        show_order_lists.place(x=150, y=20)

        show_users = Button(newWindow, text='Show customers', command=lambda: [users(newWindow)])
        show_users.place(x=300, y=20)

        show_users1 = Button(newWindow, text='BACK', command=lambda: [users(newWindow)])
        show_users1.place(x=300, y=100)
...
login_button = Button(root, text="Login", command=lambda:[login_user(root)], height=2, width=7)
login_button.place(x=250, y=130)

login_button2 = Button(root, text="Customer", command=lambda:[customer(root)], height=2, width=7)
